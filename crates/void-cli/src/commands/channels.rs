...
pub fn run(args: &ChannelsArgs, json: bool) -> anyhow::Result<()> {
    debug!(search = ?args.search, account = ?args.account, limit = args.limit, "channels");
    let cfg = VoidConfig::load_or_default(&config::default_config_path());
    let db = Database::open(&cfg.db_path())?;
    let formatter = OutputFormatter::new(json);

    let channels = db.list_channels(args.account.as_deref(), args.search.as_deref(), args.limit)?;

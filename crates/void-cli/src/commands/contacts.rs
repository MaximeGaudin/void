...
pub fn run(args: &ContactsArgs, json: bool) -> anyhow::Result<()> {
    debug!(search = ?args.search, account = ?args.account, limit = args.limit, "contacts");
    let cfg = VoidConfig::load_or_default(&config::default_config_path());
    let db = Database::open(&cfg.db_path())?;
    let formatter = OutputFormatter::new(json);

    let contacts = db.list_contacts(args.account.as_deref(), args.search.as_deref(), args.limit)?;

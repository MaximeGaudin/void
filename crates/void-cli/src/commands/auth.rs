...
pub async fn run(args: &AuthArgs) -> anyhow::Result<()> {
    info!(connector_type = %args.connector_type, account = ?args.account_name, "starting auth");
    ...
    let target_type = args.connector_type.to_lowercase();
    let account = cfg
        .accounts
        .iter()
        .find(|a| {
    ...
    let conn_mut = Arc::get_mut(&mut conn).ok_or_else(|| {
        anyhow::anyhow!("internal error: could not get mutable connector reference")
    })?;

...
async fn run_forward(args: &ForwardArgs) -> anyhow::Result<()> {
    ...
    check_forward_connector(&args.message_id, &msg.connector, "slack")?;
...
mod tests {
    ...
    #[test]
    fn forward_connector_guard_accepts_slack() {
        assert!(check_forward_connector("id1", "slack", "slack").is_ok());
    }

    #[test]
    fn forward_connector_guard_rejects_non_slack() {
        assert!(check_forward_connector("id1", "gmail", "slack").is_err());
    }

...
/// Resolve a Bot API dialog ID to a live `Peer` via ambient authority.
pub(crate) async fn resolve_numeric_peer(client: &Client, id: i64) -> anyhow::Result<Peer> {
    let peer_id = PeerId::from_bot_api_dialog_id(id)
        .ok_or_else(|| anyhow::anyhow!("invalid peer ID: {id}"))?;
    Ok(client.resolve_peer(peer_id.to_ambient_ref()).await?)
}

/// Unwrap `Peer::to_ref`, which returns `Result<Option<PeerRef>, _>` in grammers 0.10.
pub(crate) async fn peer_ref(peer: &Peer) -> anyhow::Result<PeerRef> {
    peer.to_ref()
        .await

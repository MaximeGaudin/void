...
- **MCP** — `void mcp` stdio server: named read/write tools (`inbox`, `conversations`, `messages`, `search`, `contacts`, `channels`, `slack_saved`, `calendar`, `health`, `send`, `reply`, `forward`, `archive`, `mute`), a `run` tool for full CLI parity via subprocess, and a `triage_inbox` prompt that ships the Inbox Zero workflow guide. See [docs/mcp.md](docs/mcp.md).
